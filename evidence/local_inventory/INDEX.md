...
独立复核更新：见[复核报告](../../reviews/LOCAL_INVENTORY_REVIEW_2026-09-06.md)。已验证19项上传哈希及部分结果汇总；另发现几何代码历史版本不匹配、失败点说明不一致和时间同步边界。原始证据副本及 `SHA256SUMS` 保持不变，复核产物单独保存在 `reviews/`。
...
- 初次本机整理时，校验对新增内容不报断链，但本机检出中的4份 `archive/reports/*.md` 哈希报错。后续独立复核证实远端主分支的原始字节全部匹配，撤回“main 自身原件已不匹配”的归因。自动换行检出可复现相同报错；本次加入字节保留规则后模拟检出校验通过。用户本机实际配置仍未读取，详情见上述复核报告。
